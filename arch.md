...
- [editorial_brain/brain.py](./editorial_brain/brain.py)
  - 调用 LLM 并解析结果。
...
[api_gateway/llm_client.py](./api_gateway/llm_client.py) 提供了统一的 LLM 抽象：
...
只要实现新的 `LLMClient`，并保持 `LLMRequest` / `LLMResponse` 协议兼容，即可替换当前网关实现。
