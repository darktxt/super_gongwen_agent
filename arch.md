...
- 使用 `agents_runtime` 在受控边界内读取材料并执行 SDK tools；
...
agents_runtime/         OpenAI Agents SDK 运行时适配、materials 工具与底层访问
...
- 控制类 action：`load_skill`、`ask_user`
- 写作类 action：`build_outline`、`write_draft`、`write_section`、`revise_draft`、`polish_language`、`finalize`

材料检索与读取不再作为独立 `read_materials` action 由应用层手工串联，而是通过 Agents SDK 原生 `function_tool` 在单轮运行中完成；模型在调用工具后继续返回最终业务 action。
...
### 4.4 Agents Runtime Tools 层

`agents_runtime/` 统一承载模型运行时适配、材料工具、materials 底层访问与结果落盘，不再保留旧的独立工具目录。

关键模块：

- [agents_runtime/materials_fs.py](./agents_runtime/materials_fs.py)
  - 真正负责材料目录解析、文件遍历、文本读取、PDF/DOCX 提取。
- [agents_runtime/result_store.py](./agents_runtime/result_store.py)
  - 在工具结果过大时落盘为引用，避免会话目录内联结果过长。

当前主链路中，模型可见的材料工具直接定义在 `agents_runtime/` 的 Agents SDK 原生 `function_tool` 中，并由 SDK 自身执行 tool loop；同目录下的 `materials_fs.py` 与 `result_store.py` 负责底层受控访问、结果落盘与安全边界校验。
...
4. 调用 Agents SDK 运行时请求模型；若需要补材料，则由 SDK 在同一轮内直接调用 `function_tool`。
5. 将工具结果回写到工作区，再把模型最终输出解析为 `BrainStepResult`。
6. 若动作为 `load_skill`，则通过 `SkillTool` 加载 skill。
7. 将技能结果、工具结果或模型 patch 回写到工作区。
8. 若动作为 `done`，则先通过 `QualityGate`，通过后保存最终输出。
9. 全程记录事件、调试文件、版本文件，并持久化 `workspace.json`。
...
SuperGongwenApp.run_turn()
   ...
   |      |
   |      +--> SDK function_tool (search/list/read/grep)
   |
   +--> SkillTool
   |
   +--> QualityGate
...
- [agents_runtime/materials_fs.py](./agents_runtime/materials_fs.py) 会优先解析项目根目录下的 `materials/`。
...
如需新增材料工具，应优先在 `agents_runtime/tools.py` 中新增 SDK `function_tool`，并在必要时扩展 `agents_runtime/materials_fs.py` 的底层访问能力。
