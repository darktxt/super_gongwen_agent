...
def build_editorial_system_prompt() -> str:
    ...
    return "\n".join(
        [
            f"你是 super-gongwen-agent 的中文公文写作主控 agent。你的首要目标是在当前条件下尽可能完成用户任务，并输出合法 JSON。可选 action：{actions}。",
            "你必须基于工作区事实、已读材料、最近自审和历史阻断信息作出策略判断，但不要把中间字段是否齐全误当作任务是否完成。",
            "如果材料不完备，可以采用保守可交付、继续修订或 ask_user 等策略；不要为了流程完整而空转，也不要把推断写成既成事实。",
            "正文质量、结构完整度、正式语体、措施可执行性和风险披露，优先于辅助字段是否齐全。",
            "不要因为 evidence_board、quality_backlog、self_review 等辅助状态未满就自动否决交付；只要正文在当前边界内可用，就应优先完成任务。",
            "最终只输出一个 JSON object。",
            "输出顶层允许 action_taken、action_payload、workspace_patch、self_review、business_completion_declared、completion_mode、decision_rationale、assumptions、major_risks。",
            ...
            "当你判断可以交付时，要写清 decision_rationale、completion_mode，以及必要的 assumptions 和 major_risks。",
        ]
    )
...
- 只有在稿件已经适合正式交付或适合以保守方式交付时才 `finalize`
- ask_user 只用于关键信息缺失且不能自行补齐的情况
- 是否 finalize 主要看正文是否已经达到可交付标准，而不是辅助字段数量是否够多
[公文质量]
- 保持正式、准确、克制的中文公文表达
- 优先写清事实依据、问题回应、措施抓手和责任要求
- 材料不足时优先用保守措辞、假设说明和风险披露完成任务"""
...
class ContextCompiler:
    ...
    def build(self, snapshot: WorkspaceSnapshot) -> CompiledBrainContext:
        ...
        return CompiledBrainContext(
            ...
            action_playbook_block=self._block(
                "Action Playbook",
                action_playbook,
                budget,
                prefer_full=True,
            ...
            attached_context_blocks=[
                ...
                self._json_block(
                    ...
                    {
                        "has_outline": bool(
                            snapshot.current_outline.outline_text
                            or snapshot.current_outline.sections
                        ),
                        ...
                        "must_follow_checks": {
                            ...
                            "placeholder_count": str(snapshot.current_draft.full_text or "").count(
                                "XX"
                            ),
                ...
                self._json_block(
                    "Material Catalog",
                    {
                        ...
                        "items_preview": [
                            item.to_dict() for item in snapshot.material_catalog.items[:8]
                        ],
                    },
                    budget,
                ),
                self._json_block(
                    "Retrieved Materials",
                    {
                        ...
                        "recent_excerpts": [
                            item.to_dict()
                            for item in snapshot.retrieved_materials.excerpts[-6:]
                        ],
                    },
                    budget,
                    prefer_full=True,
                ...
                self._json_block(
                    "Current Draft and Outline",
                    {
                        ...
                        "outline_sections": [
                            item.to_dict() for item in snapshot.current_outline.sections
                        ],
                ...
                self._json_block(
                    "Current Self Review",
                    snapshot.current_self_review.to_dict(),
                    budget,
                ),
                self._json_block(
                    "Recent Revision History",
                    [item.to_dict() for item in snapshot.revision_history[-6:]],
                    budget,
                ),
                ...
                self._json_block(
                    "Available Tools",
                    {
                        ...
                        "tools": [
                            dict(tool) for tool in snapshot.available_tools if isinstance(tool, dict)
                        ],
                    },
                    budget,
                ),
    ...
    def _build_user_prompt(self, snapshot: WorkspaceSnapshot) -> str:
        ...
        return "\n".join(
            [
                ...
                "优先关注：如何在当前条件下最好地完成任务、当前稿件是否可交付、哪些风险需要显式披露、是否真的需要追问用户。",
            ]
        )
    ...
    def _blockers(self, snapshot: WorkspaceSnapshot) -> list[str]:
        blockers = [
            str(item).strip()
            for item in list(snapshot.finalization_blockers or [])
            if str(item).strip()
        ]
        if blockers:
            return blockers[:5]
        return [
            ...
            for item in snapshot.quality_backlog.items[:5]
            if str(item.description).strip()
        ]
    ...
    def _json_block(
        ...
        *,
        ...
        return self._block(
            title,
            json.dumps(self._normalize(payload), ensure_ascii=False, indent=2),
            budget,
